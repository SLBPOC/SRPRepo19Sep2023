...
export class WellFilterAndSortComponent implements OnInit {
  ...
  effectiveRuntimeSlider: IEffectiveRuntimeSlider = {
    min: 0,
    max: 100,
    start: 0,
    end: 100,
  };
  cyclesTodaySlider: IcyclesTodaySlider = {
    min: 0,
    max: 100,
    start: 0,
    end: 100,
  };
  structuralLoadSlider: IstructuralLoadSlider = {
    min: 0,
    max: 100,
    start: 0,
    end: 100,
  };
  minMaxLoadSlider: IminMaxLoadSlider = {
    min: 0,
    max: 100,
    start: 0,
    end: 100,
  };
  gearboxLoadSlider: IgearboxLoadSlider = {
    min: 0,
    max: 100,
    start: 0,
    end: 100,
  ...
  ngOnChanges() {
    ...
    let effectiveRuntime = this.ColumnList.find(
      (e: any) => e.value === 'effectiveRuntime'
    );
    if (!effectiveRuntime) {
    ...
    let structuralLoad = this.ColumnList.find(
      (e: any) => e.value === 'structuralLoad'
    );
    if (!structuralLoad) {
    ...
    let gearboxLoad = this.ColumnList.find(
      (e: any) => e.value === 'gearboxLoad'
    );
    if (!gearboxLoad) {
  ...
  onCategorySelection(selectedCategoryArray: any) {
    ...
    this.filtersApplied.category = selectedCategoryArray.length > 0;
    this.updateAppliedFilter();
  }

  submitAppliedFilters() {
    const commStatus = this.commsStatusOptions
    ...
    const controllerStatus = this.controllerStatusOptions
      .filter((element: any) => element.checked === true)
      .reduce((acc, element, index) => {
        acc.push(element.value);
        return acc;
    ...
    const pumpingType = this.pumpingTypeOptions
      .filter((element: any) => element.checked === true)
      .reduce((acc, element, index) => {
        acc.push(element.value);
        return acc;
