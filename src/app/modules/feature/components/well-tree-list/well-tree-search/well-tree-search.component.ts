...
export class WellTreeSearchComponent {
  ...
  constructor(
    ...
    this.treeViewService.selectedNodes.subscribe((x) => {
      this.isSaveEnabled =
        ...
        x != undefined &&
        x.length != 0 &&
        this.currentSelectedSavedSearch == null;
    });
    this.treeViewService.savedStateEmit.subscribe(
      (x) => (this.hasSavedList = x.length > 0)
    );
  ...
  onCheckBoxChange(event: any, nodeType: NodeType) {
    if (event.checked) this.selectedNodeTypes.push(nodeType);
    else
      this.selectedNodeTypes.splice(
        this.selectedNodeTypes.findIndex((x) => x == nodeType),
        1
      );
    this.onSearchChange();
  }
